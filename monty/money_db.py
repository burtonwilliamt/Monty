...
class MoneyDatabase:
    ...
    def __init__(self):
        ...
        self._balance_cache: dict[int, dict[int, float]] = self._fetch_balances()
    ...
    def _fetch_balances(self):
        ...
        for row in res:
            ...
            if user_id in balances[guild_id]:
                raise RuntimeError(f"User {user_id} has multiple latest transactions.")
    ...
    def attempt_transaction(
        self, user: discord.Member, delta: float, reason: str
    ) -> None:
        ...
        cur.execute(
            ...
            (
                ...
                user_id,
                guild_id,
                pickle.dumps(current_value, protocol=5),
                pickle.dumps(new_value, protocol=5),
                reason,
            ),
        )
        self._con.commit()
